...
// the external store, so it is read through useSyncExternalStore and the
// server snapshot is "not reduced", which matches the site's CSS default.
...
export function useSelfReveal<T extends HTMLElement>(): {
  ...
  // Reduced motion means revealed from the start, derived rather than set in
  // an effect.
  ...
  return (
    <figure
      ...
      className={[
        ...
        // The entrance is the site's own: `[data-reveal]` carries the offset,
        // the transition, the reduced-motion override and, critically, the
        // `.no-js` rule that reveals everything when the page has no script.
        // Owning opacity here instead would leave the figure invisible to a
        // reader with JavaScript off.
        revealed || reducedMotion ? "is-revealed" : "",
        className,
      ].join(" ")}
    >
      {children}
