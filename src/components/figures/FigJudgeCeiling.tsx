...
const DEFAULT_DISQUALIFIED: DisqualifiedJudge = {
  ...
  code: "COH",
  detail: "0 scorable units, 158 of 192 calls failed to parse",
};
...
export function FigJudgeCeiling({
  ...
  // A render helper rather than a nested component: a component created
  // during render gets a new type every pass, which React and this repo's
  // lint rule both reject. Same output, called as a function.
  const row = ({
    r,
    y,
    delay,
    ...
    return (
      <g opacity={tone === "faded" ? 0.75 : 1}>
        ...
        <text
          x={
            narrow
              ? // A CI reaching the right of the plot leaves no room for a
                // start-anchored label, and it ran off the card. Flip to
                // right-anchored at the plot edge instead of overflowing.
                x(r.ciHigh) + 6 > W - LEFT - 74
                ? W - LEFT
                : x(r.ciHigh) + 6
              : W - LEFT
          }
          ...
          textAnchor={
            narrow && x(r.ciHigh) + 6 <= W - LEFT - 74 ? "start" : "end"
          }
          fill={verdictColor}
          fontWeight={700}
        >
        ...
        >
          ...
          {candidates.map((r, i) => (
            <Fragment key={r.judge}>
              {row({ r, y: rowY[i], delay: 80 + i * 60, tone: "solid", verdict: "FAIL", verdictColor: EMBER })}
            </Fragment>
          ))}
          ...
          <g opacity={0.85}>
            ...
            {!narrow && (
              <text x={PADL} y={yDisq + 15} fontFamily={FONT_MONO} fontSize={8} fill={SLATE} fontStyle="italic" textAnchor="start">
                {disqualified.detail}
              </text>
            )}
